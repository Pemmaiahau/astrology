...
- `fmtDeg`/`fmtLongitude` — `14.372°` → `14°22'` display formatting. Arcminutes **truncate** (with a 1e-9 guard against binary representation error, then a clamp at 59'): every caller passes a degree-in-sign, rounding used to print an impossible `30°00'`, and flooring keeps the printed degree consistent with `signOf`/`nakshatraOf`/`padaOf`, which all floor too.
...
- `isRetrograde(id, date, nodeMode)` — speed < 0. Only the Sun and Moon are hard-coded as never retrograde: **Rahu and Ketu are read from the speed like everything else**, so the mean node comes back permanently vakri (≈ −0.053°/day, matching the classical convention) while the true node's brief direct stretches show up as such. `nodeMode` must be passed through here as well as to `dailySpeed`, or a true-node chart reports mean-node retrogression.
...
2. **Sripati Bhava Chalit** — `sripatiHouses(ascSidereal, mcSidereal)`:
   ...
   - **Degeneracy guard.** Because Dsc = Asc+180 and IC = MC+180, all four quadrant arcs follow from the first: `arc(Asc→IC) = arc(Dsc→MC) = q`, `arc(IC→Dsc) = arc(MC→Asc) = 180 − q`. So `0 < q < 180` is the single condition for a trisectable frame. It fails above the polar circle (89.9°N gives q = 350°, which used to be trisected into 117°-wide bhavas leaving 8 of 12 unreachable by `bhavaOf`) and in manual mode when a hand-picked Lagna sits far from the Ascendant the supplied MC belongs to. Both now fall back to `equalHouses(asc)` and report `method: "equal"`, surfaced on `ChartMeta.bhavaMethod` and in the chart's Chalit note. `sripatiHouses` returns a `HouseFrame { madhya, sandhi, method }`.
...
- **`applyGrahaYuddha(planets)`** — mutates planets in place: among the five *tara grahas* (Ma/Me/Ju/Ve/Sa), any pair within 1° of each other is a "planetary war"; **the lower sidereal longitude wins** — the graha behind in zodiacal order (classical texts vary on the tie-break — flag if a different convention is wanted). The comparison is on the longitude, not the degree-in-sign, because a war straddling a sign boundary puts the *higher* degInSign on the graha that is actually behind. When 3+ grahas fall inside 1°, each records its **nearest** opponent (`PlanetPosition` holds one opponent per graha), and the result is independent of array order.
...
- **Graha Yuddha winner = lower sidereal longitude** (the graha behind in zodiacal order). This is one defensible convention among a few in classical literature; if a user cites a different tie-break rule they were taught, it's a one-line change in `applyGrahaYuddha` (states.ts), not a deep fix. **Do not "simplify" this back to a degree-in-sign comparison** — that was a live bug: it awarded a cross-boundary war (Mars 29°42' Aries vs Venus 0°12' Taurus) to Venus, and `computeShadbala` then transferred the whole virupa difference the wrong way. Pinned by a Phase 7 harness check.
...
**2026-09 extension — the whole Interpretation tab.** The same voice now covers everything default-visible on the tab, not just the six cards: `personality.ts` (second person, every term glossed inline on first use — "the 'Lagna' — the sign that was coming up over the eastern horizon"), the `synthesis.ts` scaffolding sentences for the house-by-house reading (`DIGNITY_PLAIN` / `DIGNITY_INLINE` replace `DIGNITY_LABELS` in prose; `ROLE_PLAIN` / `ROLE_SHORT` replace "functional benefic"; "kendra/trikona/dusthana" are spelled out as corner / fortunate / difficult houses with the Sanskrit in brackets), the 12 `FUNCTIONAL_ROLES[].note` strings, and the curated tables themselves — `planetInHouse.ts`, `aspectTexts.ts`, `conjunctions.ts` and the `yogas.ts` descriptions had every "the native" converted to "you" and their few remaining unglossed Sanskrit terms (dig bala, purva punya, moksha-karaka, "significations") rewritten. Within one house the nakshatra and aspect explainers are given in full only on the first occurrence (`explain` flag on `nakshatraSentence` / `aspectParagraph`) and shortened after that. **The collapsed "Classical corroboration" depth block and every expert tab were deliberately left in classical vocabulary** — the rule remains: simplify the default-visible layer, keep depth opt-in and precise.
