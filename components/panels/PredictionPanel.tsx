...
export default function PredictionPanel() {
  ...
  return (
    <div className="space-y-5">
      {/* Controls */}
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-indigo-800/50 bg-indigo-950/40 p-3">
        ...
        <div className="ml-auto flex items-center gap-1 rounded-lg border border-indigo-800/60 bg-indigo-950/60 p-1">
          <button
            type="button"
            onClick={() => stepYear(-1)}
            className="rounded-md px-2 py-1 text-slate-400 transition hover:text-amber-300"
          ...
          <input
            ...
            className="w-16 bg-transparent text-center font-mono text-sm font-bold text-amber-300 outline-none"
          />
          <button
            type="button"
            onClick={() => stepYear(1)}
          ...
          {predictionYear !== now.getFullYear() && (
            <button
              ...
              className="rounded-md px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400 transition hover:text-amber-300"
            >
              Today
            </button>
          )}
      ...
      {forecast.beforeBirth && (
        <p className="rounded-xl border border-fuchsia-800/40 bg-fuchsia-950/20 p-3 text-xs text-fuchsia-300/90">
      ...
      {(forecast.mahaSegments.length > 0 || forecast.sadeSatiText) && (
        <section>
          <h4 className="mb-3 text-sm font-bold uppercase tracking-wider text-amber-500/80">Year Overview</h4>
          <div className="space-y-4">
            {forecast.mahaSegments.map((m, i) => (
              <div key={i} className="rounded-xl border border-indigo-800/50 bg-indigo-950/40 p-4">
            ...
            {forecast.sadeSatiText && (
              <div className="rounded-xl border border-indigo-800/50 bg-indigo-950/40 p-4">
                <p className="text-sm leading-relaxed text-slate-300">{forecast.sadeSatiText}</p>
              </div>
            )}
          </div>
        </section>
      ...
      {/* Segmented timeline */}
      <section>
        <h4 className="mb-3 text-sm font-bold uppercase tracking-wider text-amber-500/80">
          ...
          {forecast.timeline.map((seg, i) => (
            <div key={i} className="rounded-xl border border-indigo-800/50 bg-indigo-950/40 p-4">
              <div className="mb-2 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                ...
                <span className="rounded-full bg-indigo-500/10 px-2 py-0.5 text-[10px] font-medium text-indigo-300/80">
                  {seg.boundaryReason}
                </span>
              </div>

              {seg.antarText && (
                <p className="mb-3 text-sm leading-relaxed text-slate-300">{seg.antarText}</p>
              )}

              {seg.transits.length > 0 && (
                <div className="mb-3 space-y-2">
                  {seg.transits.map((t) => (
                    <p key={t.id} className="text-xs leading-relaxed text-slate-400">
                      <span className="font-semibold text-slate-300">
              ...
              {seg.pratyantars.length > 0 && (
                <div className="rounded-lg border border-indigo-800/40 bg-indigo-950/50 p-2.5">
                  <p className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                    Pratyantardashas in this segment
                  </p>
                  <div className="space-y-1">
                    {seg.pratyantars.map((p, j) => (
                      <p key={j} className="text-xs leading-relaxed text-slate-400">
                        <span className="font-mono text-slate-500">
                          {fmt(p.start)}–{fmt(p.end)}
                        </span>{" "}
                        <span className="font-semibold text-amber-300/80">{PLANET_NAMES[p.lord]}</span>:{" "}
                        {p.theme}
                      </p>
                    ))}
                  </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {forecast.months.map((m, i) => (
            <div key={i} className="rounded-xl border border-indigo-800/50 bg-indigo-950/40 p-3">
              <p className="mb-1 text-sm font-bold text-amber-300">{m.label}</p>
              {m.dashaLine && <p className="mb-2 text-[11px] font-medium text-slate-500">{m.dashaLine}</p>}
              {m.paragraphs.map((p, j) => (
                <p key={j} className="mb-1.5 text-xs leading-relaxed text-slate-400 last:mb-0">
                  {p}
                </p>
              ))}
            </div>
